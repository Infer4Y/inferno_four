...
public class BlockHeater extends Block {
    ...
    public BlockHeater(String name){
        super(Material.IRON);
        this.name = name;
    ...
    public TileEntityHeater getTileEntity(IBlockAccess world, BlockPos pos) {
        return (TileEntityHeater)world.getTileEntity(pos);
    }

    @Override
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            TileEntityHeater tile = getTileEntity(worldIn, pos);
